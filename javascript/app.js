...
database.ref("/train-data").on("child_added", function(update) {
    //Captures the updated data in variables again
    let trainName = update.val().trainName;
    let trainDestination = update.val().trainDestination;
    let firstTrain = update.val().firstTrain;
    let trainFrequency = update.val().trainFrequency;
